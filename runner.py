...
for abbr, date_val, ltype, period, status, remarks in inserts_data:
    ...
    c.execute("""INSERT INTO Leave (id, userId, date, period, type, status, remarks, createdAt)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)""", (leave_id, user_id, date_val, period, ltype, status, remarks, created_at))
    success += 1
