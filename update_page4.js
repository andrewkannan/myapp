...
// Use regex with matching any whitespace
const filterRegex = /\{\/\* Filter Dropdown \*\/\}[\s\S]*?<\/select>\s*<\/div>\s*\)\}/;
...
const buttonsRegex = /\{\['ADMIN', 'MANAGER'\]\.includes\(currentUser\.accessLevel\) && \([\s\S]*?<span className="hidden-mobile">Logout<\/span>\s*<\/button>/;
...
if (!content.includes('Settings')) {
  content = content.replace("import { ChevronLeft, ChevronRight, Users, Printer, LogOut }", "import { ChevronLeft, ChevronRight, Users, Printer, LogOut, Settings }");
}
